...
{
	private const string TAG = "SOOMLA FB";
	...
	{
		SoomlaProfile.PushEventLoginStarted (Provider.FACEBOOK);
		if (callback != null) {
	...
	{
		SoomlaProfile.PushEventLogoutStarted (Provider.FACEBOOK);
		FacebookImpl.Logout();
		SoomlaProfile.PushEventLogoutFinished (Provider.FACEBOOK);
	}
	...
	public static void Feed(
		...
		FacebookDelegate callback = null)
	{
		SoomlaProfile.PushEventSocialActionStarted (Provider.FACEBOOK, SocialActionType.UPDATE_STATUS);
		if (callback != null) {
	...
	{
		string api = query.Substring (0, Math.Max(query.Length, query.IndexOf ("?")));
		SoomlaUtils.LogDebug (TAG, "FB API call:" + api);
		switch (api) {
		case "/me/photos":
			SoomlaProfile.PushEventSocialActionStarted(Provider.FACEBOOK, SocialActionType.UPLOAD_IMAGE);
			if (callback != null) {
			...
			break;
		case "/me/friends":
			SoomlaProfile.PushEventSocialActionStarted(Provider.FACEBOOK, SocialActionType.GET_CONTACTS);
			if (callback != null) {
